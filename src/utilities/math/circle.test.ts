...
describe('getPoint', () => {
	it('Gets points all around the circle correctly', () => {
		...
		expect(math.circles.getPoint(90, radius)).toStrictEqual({
			x: 0,
			y: 1,
		} satisfies math.Point);
		...
		expect(math.circles.getPoint(180, radius)).toStrictEqual({
			x: 1,
			y: 2,
		} satisfies math.Point);
		...
		expect(math.circles.getPoint(270, radius)).toStrictEqual({
			x: 2,
			y: 1,
		} satisfies math.Point);
		...
		expect(math.circles.getPoint(360, radius)).toStrictEqual({
			x: 1,
			y: 0,
		} satisfies math.Point);
	});
